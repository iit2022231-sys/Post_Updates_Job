...
class PostDistributor:
    ...
    async def send_posts(self, posts: List[Post]) -> Dict[str, Any]:
        """Send posts to all destinations"""
        if not posts:
            return {"sent": 0, "failed": 0}

        config = self.config.get_posting_config()
        delay = config.get("delay_between_posts_seconds", 2)
        
        stats = {"sent": 0, "failed": 0}
        
        for post in posts:
            for dest in self.config.get_destinations():
    ...
    async def _send(self, post: Post, dest: str, retries: int = 2) -> bool:
        """Send single post with retries"""
        
        for attempt in range(retries):
            try:
                await self.client.get_client().send_message(dest, post.text, parse_mode="markdown")
